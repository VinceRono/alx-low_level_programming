 ...
 * Description: Magic no's separated by spaces.
 */
void print_magic(unsigned char *e_ident)
{
	int indx;
 ...
 * @e_ident: pointer to array containing ELF ver.
 */
void print_osabi(unsigned char *e_ident)
{
	printf(" OS/ABI: ");
...
/**
 * print_abi - Prints ABI ver of ELF header.
 * @e_ident: pointer to array containing ELF ABI ver.
 */
void print_abi(unsigned char *e_ident)
{
...
int main(int __attribute__((__unused__)) argc, char *argv[])
{
	...
	rn = read(on, header, sizeof(Elf64_Ehdr));
	if (rn == -1)
	{
		free(header);
		close_elf(on);
